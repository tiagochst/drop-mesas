...
void indice_deleta(int id){  
  ...
  if(pos == -1){
    ...
    for(i=pos;i<N_IEspec;i++)  IEspec[i] = IEspec[i+1];
    N_IEspec--;
    IEspec = (Indice_Prim *)realloc(IEspec,(N_IEspec)*sizeof(Indice_Prim));
...
int indice_busca(int id){
  ...
  if(IEspec[ini].id > id || IEspec[fim].id < id)  return -1;
  
  if(IEspec[ini].id == id) return ini;
  if(IEspec[fim].id == id) return fim;
  
  while(fim - ini >= 2){ 
    meio = (ini + fim)/2;
    if(IEspec[meio].id > id) fim = meio; 
    else if(IEspec[meio].id < id) ini = meio;
    else return meio;      
  }
