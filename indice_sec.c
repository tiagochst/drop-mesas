...
void indice_sec_start(char *indiv, char *captu) {
	...
	if (FISecIndiv != NULL) {
		...
		} else { /*caso contrario, le do arquivo de indice*/
		...
		if (flag == FAIL) {
			...
			while (i--) {
				...
				if (Y.idC == FAIL)
					...
					indice_sec_insere("captura", Y.idI, offset);
			}
		} else {
 ...
 * idP (o outro atributo), mantendo esse vetor ordenado
 * por idS*/
void indice_sec_insere(char *op, int idS, int idP) {
	...
	if (!strcmp(op, "individuo")) {
		N_ISIndiv++;
		ISIndiv = (Indice_Sec *) realloc(ISIndiv, (N_ISIndiv)
				* sizeof(Indice_Sec));
		i = N_ISIndiv - 1;
	...
	if (!strcmp(op, "captura")) {
		N_ISCaptu++;
		ISCaptu = (Indice_Sec *) realloc(ISCaptu, (N_ISCaptu)
				* sizeof(Indice_Sec));
		i = N_ISCaptu - 1;
...
/* Realiza uma busca binaria em, retornando o primeiro elemento
 * que tenha como identificador id */
